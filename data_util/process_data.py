...
parser.add_argument("--id", type=str, default="obama", help="identity of target person")
parser.add_argument("--step", type=int, default=0, help="step for running")
...
if running_step == 0:
    ...
    extract_ds_cmd = (
        "python data_util/deepspeech_features/extract_ds_features.py --input=" + id_dir
    )
    ...
    for image_path in os.listdir(ori_imgs_dir):
        if image_path.endswith(".jpg"):
            ...
            if len(preds) > 0:
                ...
                np.savetxt(
                    os.path.join(ori_imgs_dir, image_path[:-3] + "lms"), lands, "%f"
                )
...
if running_step == 3:
    ...
    face_parsing_cmd = (
        "python data_util/face_parsing/test.py --respath=dataset/"
        + id
        + "/parsing --imgpath=dataset/"
        + id
        + "/ori_imgs"
    )
...
if running_step == 6:
    ...
    est_pose_cmd = (
        "python data_util/face_tracking/face_tracker.py --idname="
        + id
        + " --img_h="
        + str(h)
        + " --img_w="
    ...
    print(f"Estimate Head Pose Command: {est_pose_cmd}")
...
if running_step == 7:
    ...
    for i in range(2):
        ...
        for i in ids:
            ...
            lms = np.loadtxt(os.path.join(ori_imgs_dir, str(valid_img_ids[i]) + ".lms"))
    ...
    Path(os.path.join(dir_path, "dataset", id, "logs", id + "_head")).mkdir(
    ...
    with open(ComNeRF_config_file, "w") as file:
        ...
        file.write("far = " + str(mean_z + 0.4) + "\n")
        file.write("testskip = " + str(testskip) + "\n")
    Path(os.path.join(dir_path, "dataset", id, "logs", id + "_com")).mkdir(
        parents=True, exist_ok=True
    )
